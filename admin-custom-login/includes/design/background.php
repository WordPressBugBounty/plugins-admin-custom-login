...
<div class="row">
	...
	<div class="panel panel-primary panel-default content-panel col-md-12">
		<div class="panel-body">
			<div class="row flex-align-center">
							...
							<td>
								<select id="select-background" class="standard-dropdown" name="select-background" onchange='getComboid()'>
									...
									<optgroup label="<?php esc_html_e('Select Background', 'admin-custom-login') ?>">
										<option value="static-background-color" <?php if ($top_bg_type == 'static-background-color') {
																					echo "selected";
																				} ?>><?php esc_html_e('Static Background Color', 'admin-custom-login') ?></option>
										<option value="static-background-image" <?php if ($top_bg_type == 'static-background-image') {
																					echo "selected";
																				} ?>><?php esc_html_e('Static Background Image', 'admin-custom-login') ?></option>
									</optgroup>
								</select>
							</td>
						</tr>
					</table>
					<div class="save-button-block margin-t-20 d-none" id="no-bg-save-btn">
				...
				<div class="col-md-6 preview-column">
					<div id="preview-bg-color" class="no-background w-100">
						<img src="<?php echo esc_url(WEBLIZAR_NALF_PLUGIN_URL . '/images/background-color1.png'); ?>" class="img-responsive preview-img" alt="">
					</div>
					<div id="preview-bg-image" class="no-background w-100">
						<img src="<?php echo esc_url(WEBLIZAR_NALF_PLUGIN_URL . '/images/background-image.png'); ?>" class="img-responsive preview-img">
					</div>
					<div id="preview-bg-slideshow" class="no-background w-100">
						<img src="<?php echo esc_url(WEBLIZAR_NALF_PLUGIN_URL . '/images/background-slideshow.png'); ?>" class="img-responsive preview-img">
					</div>
				</div>
			</div>
		</div>
	...
	<div id="div-bakground-color" class="no-background clear-both w-100">
		<div class="panel panel-primary panel-default content-panel col-md-12">
			<div class="panel-body">
				<table class="form-table">
					<tr>
	...
	<div id="div-bakground-image" class="no-background clear-both w-100">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body">
											<img class="show_prev_img w-100-h-50" src="" id="top_img_prev" />
										</div>
		...
		<div class="panel panel-primary panel-default content-panel col-md-6">
			<div class="panel-body">
				<table class="form-table">
					...
					<tr class="radio-span">
						<td>
							<input type="checkbox" value="yes" id="bg-strech" name="strech-bg" onclick="OnChangeCheckbox(this)" class="v-visible" <?php if ($top_cover == "yes") {
																																									echo esc_attr("checked");
																																								} ?> />&nbsp;<label for="bg-strech"><?php esc_html_e('Yes', 'admin-custom-login') ?></label>
						</td>
					</tr>
				</table>
			</div>
		...
		<div class="save-button-block mt-24">
			<button type="button" onclick="return Custom_login_top('topbgSave', '');" class="btn btn-info btn-lg"><?php esc_html_e('Save Changes', 'admin-custom-login') ?></button>
			<button type="button" onclick="return Custom_login_top('topbgReset', '');" class="btn btn-primary btn-lg"><?php esc_html_e('Reset Default', 'admin-custom-login') ?></button>
		</div>
		...
		<div class="panel panel-primary panel-default content-panel">
			<div class="panel-body">
				<table class="form-table">
					...
					<tr class="radio-span">
						<td class="w-100" id="tdslider">
							<div class="row">
								<?php
								$Slidshow_image = maybe_unserialize(get_option('Admin_custome_login_Slidshow'));
								for ($i = 1; $i <= 6; $i++) {
								?>
									<div class="col-md-4 slideshow_settings <?php if ($i > $top_slideshow_no) { echo 'd-none'; } ?>" id="slideshow_settings_<?php echo esc_attr($i); ?>">
										<div class="rpg-image-entry" id="rpg_img">
											<img src="<?php if ($Slidshow_image['Slidshow_image_' . $i] != "") {
															echo esc_attr($Slidshow_image['Slidshow_image_' . $i]);
														} else {
															echo  esc_url(WEBLIZAR_NALF_PLUGIN_URL . 'images/rpg-default.jpg');
														} ?>" class="rpg-meta-image" alt="" id="simages-<?php echo esc_attr($i); ?>">
											...
											<input type="text" id="rpg_img_url-<?php echo esc_attr($i); ?>" name="rpg_img_url-<?php echo esc_attr($i); ?>" class="rpg_label_text-<?php echo esc_attr($i); ?> d-none" value="" readonly="readonly" />
										</div>
									</div>
								<?php
								} // end of foreach
	...
	<div id="somedialog1" class="dialog">
		...
		<div class="dialog__content">
			...
			<div class="dialog-inner">
				<h2><strong><?php esc_html_e('Top Background', 'admin-custom-login') ?></strong> <?php esc_html_e('Setting Save Successfully', 'admin-custom-login') ?></h2>
				<div><button class="action dialog-button-close" id="dialog-close-button1" data-dialog-close><?php esc_html_e('Close', 'admin-custom-login') ?></button></div>
			</div>
		</div>
	</div>
	...
	<div id="somedialog11" class="dialog">
		...
		<div class="dialog__content">
			...
			<div class="dialog-inner">
				<h2><strong><?php esc_html_e('Top Background', 'admin-custom-login') ?></strong> <?php esc_html_e('Setting Reset Successfully', 'admin-custom-login') ?></h2>
				<div><button class="action dialog-button-close" data-dialog-close id="dialog-close-button11"><?php esc_html_e('Close', 'admin-custom-login') ?></button></div>
			</div>
		</div>
	</div>
...
if (isset($_POST['Action'])) {
	...
	if (! wp_verify_nonce($_POST['nonce_ajax'], 'weblizar_admin_nonce')) {
		...
		//Save
		if ($Action == "topbgSave") {
			$select_bg_value 	     = (isset($_POST["select_bg_value"]) ? sanitize_text_field($_POST["select_bg_value"]) : "");
			$top_background_color    = isset($_POST['top_background_color']) ? (isset($_POST["top_background_color"]) ? sanitize_hex_color($_POST["top_background_color"]) : "") : NULL;
			$top_bg_image      	     = isset($_POST['top_bg_image']) ? (isset($_POST["top_bg_image"]) ? sanitize_text_field($_POST["top_bg_image"]) : "") : NULL;
			$top_cover 		   	     = isset($_POST['top_cover']) ? (isset($_POST["top_cover"]) ? sanitize_text_field($_POST["top_cover"]) : "") : NULL;
			$top_bg_repeat     	     = isset($_POST['top_bg_repeat']) ? (isset($_POST["top_bg_repeat"]) ? sanitize_text_field($_POST["top_bg_repeat"]) : "") : NULL;
			$top_bg_position   	     = isset($_POST['top_bg_position']) ? (isset($_POST["top_bg_position"]) ? sanitize_text_field($_POST["top_bg_position"]) : "") : NULL;
			$top_bg_attachment 	     = isset($_POST['top_bg_attachment']) ? (isset($_POST["top_bg_attachment"]) ? sanitize_text_field($_POST["top_bg_attachment"]) : "") : NULL;
			$top_slideshow_no  	     = isset($_POST['top_slideshow_no']) ? (isset($_POST["top_slideshow_no"]) ? sanitize_text_field($_POST["top_slideshow_no"]) : "") : NULL;
			$top_bg_slider_animation = isset($_POST['top_bg_slider_animation']) ? (isset($_POST["top_bg_slider_animation"]) ? sanitize_text_field($_POST["top_bg_slider_animation"]) : "") : NULL;

			$Slidshow_image_1 		 = isset($_POST['Slidshow_image_1']) ? (isset($_POST["Slidshow_image_1"]) ? sanitize_text_field($_POST["Slidshow_image_1"]) : "") : NULL;

			$Slidshow_image_2 		 = isset($_POST['Slidshow_image_2']) ? (isset($_POST["Slidshow_image_2"]) ? sanitize_text_field($_POST["Slidshow_image_2"]) : "") : NULL;

			$Slidshow_image_3 		 = isset($_POST['Slidshow_image_3']) ? (isset($_POST["Slidshow_image_3"]) ? sanitize_text_field($_POST["Slidshow_image_3"]) : "") : NULL;

			$Slidshow_image_4 		 = isset($_POST['Slidshow_image_4']) ? (isset($_POST["Slidshow_image_4"]) ? sanitize_text_field($_POST["Slidshow_image_4"]) : "") : NULL;

			$Slidshow_image_5 		 = isset($_POST['Slidshow_image_5']) ? (isset($_POST["Slidshow_image_5"]) ? sanitize_text_field($_POST["Slidshow_image_5"]) : "") : NULL;

			$Slidshow_image_6 		 = isset($_POST['Slidshow_image_6']) ? (isset($_POST["Slidshow_image_6"]) ? sanitize_text_field($_POST["Slidshow_image_6"]) : "") : NULL;

			$image_label_1 = isset($_POST['image_label_1']) ? (isset($_POST["image_label_1"]) ? sanitize_text_field($_POST["image_label_1"]) : "") : NULL;
			$image_label_2 = isset($_POST['image_label_2']) ? (isset($_POST["image_label_2"]) ? sanitize_text_field($_POST["image_label_2"]) : "") : NULL;
			$image_label_3 = isset($_POST['image_label_3']) ? (isset($_POST["image_label_3"]) ? sanitize_text_field($_POST["image_label_3"]) : "") : NULL;
			$image_label_4 = isset($_POST['image_label_4']) ? (isset($_POST["image_label_4"]) ? sanitize_text_field($_POST["image_label_4"]) : "") : NULL;
			$image_label_5 = isset($_POST['image_label_5']) ? (isset($_POST["image_label_5"]) ? sanitize_text_field($_POST["image_label_5"]) : "") : NULL;
			$image_label_6 = isset($_POST['image_label_6']) ? (isset($_POST["image_label_6"]) ? sanitize_text_field($_POST["image_label_6"]) : "") : NULL;
