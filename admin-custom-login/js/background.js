...
jQuery(document).ready(function () {
	...
	getComboid();
	setTimeout(getComboid, 100);
	setTimeout(getComboid, 500);
...
function updateBgPreviews() {
	var val = jQuery("#select-background").val();
	jQuery("#preview-bg-color, #preview-bg-image, #preview-bg-slideshow").hide();
	if (val === "static-background-color") {
		jQuery("#preview-bg-color").show();
	} else if (val === "static-background-image") {
		jQuery("#preview-bg-image").show();
	} else if (val === "slider-background") {
		jQuery("#preview-bg-slideshow").show();
	}
}
...
function Custom_login_top(Action, id) {
	...
	if (Action == "topbgReset") {
		...
		// Show Background Image Option instantly
		jQuery(".no-background").hide();
		jQuery("#div-bakground-image").show();
